...
def load5foldData(obj):
    if 'Twitter' in obj:
        labelPath = os.path.join(r'H:\\test\\resGCN(1)\\resGCN\\',"data/" +obj+"/"+ obj + "_label_All.txt")
        ...
        random.shuffle(T)
        random.shuffle(U)
        #到上述目标是分出各个标签都有什么事件，并每个标签所属事件的数量
        #分出训练集和测试集、验证集
        ...
        leng1 = int(l1 * 0.2)   #非谣言
        leng2 = int(l2 * 0.2)   #错误信息
        leng3 = int(l3 * 0.2)   #真实信息
        leng4 = int(l4 * 0.2)   #不确定信息
    ...
    if obj == "Weibo":
        labelPath = os.path.join(r'C:\Users\admin\Downloads\BiGCN-master',"data/Weibo/weibo_id_label.txt")
