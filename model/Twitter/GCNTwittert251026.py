...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model=256, max_len=5000):
        ...
        self.encoding = th.zeros(max_len, d_model)
        position = th.arange(0, max_len, dtype=th.float).unsqueeze(1)
        div_term = th.exp(th.arange(0, d_model, 2).float() * (-th.log(th.tensor(10000.0)) / d_model))
        self.encoding[:, 0::2] = th.sin(position * div_term)
        self.encoding[:, 1::2] = th.cos(position * div_term)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, input_dim, num_heads, use_positional_encoding=True):
...
class TDrumorGCN(nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_heads, use_positional_encoding=True):
        ...
        self.transformer_encoder = nn.ModuleList([
            TransformerEncoderLayer(hid_feats, num_heads, use_positional_encoding)
            for _ in range(2)
        ])
        ...
        if self.use_positional_encoding:
            self.pos_encoder = PositionalEncoding(hid_feats)
    ...
    def forward(self, data):
        ...
        if self.use_positional_encoding:
            padded_x = self.pos_encoder(padded_x)
...
class Net(nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_heads=4, use_positional_encoding=True):
        super(Net, self).__init__()
        self.TDrumorGCN = TDrumorGCN(in_feats, hid_feats, out_feats, num_heads, use_positional_encoding)
