import os
from Process.dataset import GraphDataset,BiGraphDataset,UdGraphDataset,OneGraphDataset
cwd=os.getcwd()
...
def loadTree(dataname):
    if 'Twitter' in dataname:
        treePath = os.path.join(r"H:\\test\\resGCN(1)\\resGCN\\",'data/'+dataname+'/data.TD_RvNN.vol_5000.txt')
        ...
        for line in open(treePath):
            ...
            if not treeDic.__contains__(eid):
                treeDic[eid] = {}
            treeDic[eid][indexC] = {'parent': indexP, 'max_degree': max_degree, 'maxL': maxL, 'vec': Vec}    #indexP父节点  maxL数的最大深度  max_degree最大的度  vec链接的向量边
...
def loadBiData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate,BUdroprate):
    data_path = os.path.join(r"H:\\test\\resGCN(1)\\resGCN\\",'data', dataname + 'graph')
    print("loading train set", )
...
def loadOneData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate):
    data_path = os.path.join(r"H:\\test\\resGCN(1)\\resGCN\\",'data', dataname + 'graph')
    print("loading train set", )
    traindata_list = OneGraphDataset(fold_x_train, treeDic, tddroprate=TDdroprate, data_path=data_path)
    print("train no:", len(traindata_list))
    print("loading test set", )
    testdata_list = OneGraphDataset(fold_x_test, treeDic, data_path=data_path)
    print("test no:", len(testdata_list))
    return traindata_list, testdata_list
