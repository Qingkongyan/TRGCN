...
class TDrumorGCN(th.nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_transformer_layers=1, num_heads=4, dropout=0.5):
        super(TDrumorGCN, self).__init__()

        self.embedding = th.nn.Linear(in_feats, hid_feats)  # Linear embedding layer
        self.tanh = th.nn.Tanh()  # Tanh activation

        # Transformer encoder layer
        encoder_layers = TransformerEncoderLayer(d_model=hid_feats, nhead=num_heads, dropout=dropout)
    ...
    def forward(self, data):
        ...
        x = F.dropout(x, training=self.training)
        x = F.relu(x)
        x = scatter(x, data.batch, dim=0, reduce='mean')  # Global mean pooling
        return x

class Net(nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_heads=4, num_layers=2):
        super(Net, self).__init__()
        self.TDrumorGCN = TDrumorGCN(in_feats, hid_feats, out_feats, num_heads, num_layers)
