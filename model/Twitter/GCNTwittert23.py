...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        # 假设 x 的形状为 (batch_size, seq_length, feature_dim)
...
class TDrumorGCN(nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_heads):
        ...
        self.transformer_encoder = TransformerEncoderLayer(hid_feats, num_heads)
    ...
    def forward(self, data):
        ...
        # 应用 Transformer 编码器，残差已在内部处理
        x = self.transformer_encoder(padded_x)
        x = self.transformer_encoder(padded_x)

        # 提取 Transformer 编码后相关的节点嵌入
        x = th.cat([x[i, :nodes] for i, nodes in enumerate(num_nodes)], dim=0)
...
class Net(nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_heads=4):
        ...
        self.layer_norm = nn.LayerNorm(4)  # 确保与 self.fc 的输出维度匹配
